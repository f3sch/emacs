...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  (let ((path (expand-file-name "eln-cache/" ++cache-dir)))
...
#+BEGIN_SRC emacs-lisp
  (use-package gnutls
    ;; no type built-in, straight not yet available
    :custom
...
And finally, the prompt that asks you if you want to kill a buffer with a live
...
By default, Emacs thinks a sentence is a full-stop followed by 2 spaces. Let’s
...
** Path from shell
On Linux (MacOS) Emacs doesn't use the shell PATH if it's not started from the shell.
Let's fix that:
...
#+BEGIN_SRC emacs-lisp
  (use-package doom-modeline
    ...
    (doom-modeline-env-load-string truncate-string-ellipsis)
...
#+BEGIN_SRC emacs-lisp
  (use-package modus-themes
    :straight (modus-themes :host sourcehut :repo "protesilaos/modus-themes")
    :config
    (setq
...
* Evil
At some point I got used to =vim= keybindings. Now, I cannot go back.
...
Furthermore, =consult-project-extra= only depends on consult, resulting in a lean
and simple to maintain functionality.
...
*** Buffer Relative Name
A minimal minor-mode that sets project relative paths for buffer names with
optional path abbreviation.

#+BEGIN_SRC emacs-lisp
  (use-package buffer-name-relative
    :custom
    (buffer-name-relative-fallback 'absolute)
    (buffer-name-relative-abbrev-limit 80)
    :config
...
#+BEGIN_SRC emacs-lisp
  (use-package forge
    ...
    (forge-database-file (concat +state-dir "forge/database.sqlite"))
...
#+BEGIN_SRC emacs-lisp
  (use-package forge-search
    :straight (forge-search :type git :host github :repo "eatse21/forge-search.el")
    :after forge
    :general
...
#+BEGIN_SRC emacs-lisp
  (use-package code-review
    :disabled t
    :straight (code-review :host github :repo "phelrine/code-review"
                     :branch "fix/closql-update")
    :after (magit forge)
    :custom
    (code-review-db-database-file (concat +data-dir "code-review/database.sqlite"))
...
#+BEGIN_SRC emacs-lisp
  (use-package bug-reference
    :straight (bug-reference :type built-in)
...
#+BEGIN_SRC emacs-lisp
  (use-package org-super-agenda
    ...
    :after org
    :custom
    (org-agenda-custom-commands '(("z" "Super zaen view"
    ...
    (defun +treesit-install-grammars ()
      ...
      (let ((languages (mapcar 'car treesit-language-source-alist)))
        (dolist (lang languages)
          (message "Installing Treesitter grammar: %s" lang)
          (treesit-install-language-grammar lang)
          (message "Successfully installed: %s" lang)
          (sit-for 1))))
    (+register-build-function #'+treesit-install-grammars))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package prism
    :disabled t
    :straight (prism :type git :fetcher github :repo "alphapapa/prism.el")
...
#+BEGIN_SRC emacs-lisp
  (use-package asm-mode
    :straight (asm-mode :type built-in)
...
#+BEGIN_SRC emacs-lisp
  (use-package pp
    ...
    (+local-leader-key
      ...
      "e" '(:ignore t :wk "Evaluate Code")
      "eM" '(pp-macroexpand-expression :wk "Macroexpand Expression")
      "em" '(pp-macroexpand-last-sexp :wk "Macroexpand Last SEXP")
      "eb" '(pp-buffer :wk "Eval Buffer")
      "ex" '(pp-eval-last-sexp :wk "Eval Last SEXP")
      "eX" '(pp-eval-expression :wk "Eval Expression")))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package checkdoc
    :straight (checkdoc :type built-in)
    :init
...
#+BEGIN_SRC emacs-lisp
  (use-package makem
    :disabled t
    :straight (makem :type git :host github :repo "alphapapa/makem.sh")
    :general
    (+local-leader-key
      :keymaps 'emacs-lisp-mode-map
      "M" '(makem :wk "Makem")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package cmake-font-lock
    :straight (cmake-font-lock :host github :repo "Lindydancer/cmake-font-lock" :files (:defaults "*"))
...
#+BEGIN_SRC emacs-lisp
  (use-package rustic
    ...
    (+local-leader-key
      ...
      "bd" '(rustic-cargo-build-doc :wk "Cargo build Docs")
      "bD" '(rustic-cargo-doc :wk "Cargo open Docs")
      "bf" '(rustic-cargo-fmt :wk "Cargo Format Workspace")
      "bs" '(rustic-cargo-spellcheck :wk "Cargo Spellcheck")
      "bn" '(rustic-cargo-new :wk "Cargo New")
      "bi" '(rustic-cargo-init :wk "Cargo Init")
      "bo" '(rustic-cargo-outdated :wk "Cargo Outdated")
      "br" '(rustic-cargo-run :wk "Cargo Run")

      "e" '(:ignore t :wk "Edit Cargo.toml")
      "ea" '(rustic-cargo-add :wk "Add Crate")
      "er" '(rustic-cargo-rm :wk "Remove Crate")
      "eb" '(rustic-cargo-add-missing-dependencies :wk "Add missing Dependencies")

      "E" '(rustic-cargo-epand :wk "Expand Macro")

      "t" '(:ignore t :wk "Test")
      "ta" '(rustic-cargo-test :wk "Run All")
      "tt" '(rustic-cargo-current-test :wk "Run Current")))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package devdocs
    :custom
    (devdocs-data-dir (expand-file-name "devdocs" +data-dir)))
#+END_SRC
        ...
        (with-temp-buffer
          (insert
           (shell-command-to-string
            (format "make -f %s -nqp __BASH_MAKE_COMPLETION__=1 .DEFAULT 2>/dev/null" makefile)))
          (goto-char (point-min))
          (unless (re-search-forward "^# Files" nil t)
            (error "Could not parse database!"))
          (while (re-search-forward "^\\([^%$:#\n\t ]+\\):\\([^=]\\|$\\)" nil t)
            ...
            (unless (or (save-excursion
              ...
              (push (cons (format "make:%s" target) (format "make %s" target))
                    targets))))
        targets))
...
#+BEGIN_SRC emacs-lisp
  (use-package tramp
    ...
    ;;; Profiles
    ;; Do not use authinfo
    (connection-local-set-profile-variables 'remote-without-auth-sources
                                            '((auth-sources . nil)))
  ...
  handled appropriately."
    ...
    (let ((flyspell-correct
      ...
      (general-define-key
       ...
       [return] #'flyspell-correct-wrapper
...
#+begin_src emacs-lisp
  (use-package command-log
    ...
    :commands command-log-toggle
    :custom
    (command-log-window-size 100)
...
#+BEGIN_SRC emacs-lisp
  (use-package debbugs
    :straight (debbugs :type built-in)
    :defer t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package gnus-art
    :after gnus
    :straight (gnus-art :type built-in)
    :custom
    (gnus-treat-strip-cr t))

  (use-package gnus-agent
    :after gnus
    :straight (gnus-agent :type built-in)
    :custom
    (gnus-agent-synchronize-flags 'ask))

  (use-package gnus-msg
    :after gnus
    :straight (gnus-msg :type built-in))


  (use-package gnus-sum
    :after gnus
    :straight (gnus-sum :type built-in)
    :config
    (setq-default
...
** WithEditor
This library makes it possible to reliably use the Emacsclient as the =$EDITOR= of
child processes. It makes sure that they know how to call home. For remote
...
async-shell-command and shell-command. They automatically export $EDITOR making
sure the executed command uses the current Emacs instance as “the editor”. With
a prefix argument these commands prompt for an alternative environment variable
such as =$GIT_EDITOR=. To always use these variants add this to your init file:

#+BEGIN_SRC emacs-lisp
  (use-package with-editor
    :hook
    ((shell-mode eshell-mode vterm-mode) . with-editor-export-editor))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package mm-encode
    :straight (mm-encode :type built-in)
    :custom
...
#+BEGIN_SRC emacs-lisp
  (use-package mml-sec
    :straight (mml-sec :type built-in)
    :custom
    (mml2015-use 'epg)
...
=screenshot.el= is a nice utility package made by TEC. It allows the user to take
a screenshot of a specific area of a buffer and make it look nice.
...
#+BEGIN_SRC emacs-lisp
  (+leader-key
    ...
    "d" '(devdocs-lookup :wk "Search Documentation")
    "D" '(devdocs-install :wk "Dowload Documentation")
...
#+BEGIN_SRC emacs-lisp
  (+leader-key
    ...
    "" '(:ignore t :wk "Open")
    "m" '(mu4e :wk "Mail")
    "t" '(vterm-toggle :wk "Terminal toggle")
...
#+BEGIN_SRC emacs-lisp
  (+leader-key
    ...
    "K" '(command-log-toggle :wk "Command Log")
...
** Add-hook
Emacs considers some file local variables unsafe, e.g., =after-save-hook=. Hence,
I add it as a safe pair, with =+tangle-config= function, to the
